...
-- XPerl_UnregisterPerlFrames(frame)
-- Removes a frame from the update list. Used by the all-in-one module manager
-- when a module is disabled, so the core no longer re-shows/updates its frames.
function XPerl_UnregisterPerlFrames(frame)
