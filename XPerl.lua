...
-- XPerl_IsCrossFactionAlly(unit)
-- Servers may allow Horde and Alliance in the same party/raid. The client still treats
-- those group members as hostile, so helpful spells and interact distance can't be used
-- to measure range for them.
function XPerl_IsCrossFactionAlly(unit)
	if (UnitIsPlayer(unit) and (UnitInParty(unit) or UnitInRaid(unit))) then
...
-- XPerl_InGroupRange(unit)
-- The group roster's 40 yard flag is the only range signal that ignores faction.
-- Returns 1 or 0 to match the spell/item range checks.
function XPerl_InGroupRange(unit)
...
-- XPerl_CheckInteractDistance(unit, index)
-- Inspect/trade/duel/follow are all refused across factions, so they report out of range
-- at any distance for a cross faction ally. Fall back to the roster check for those.
function XPerl_CheckInteractDistance(unit, index)
	if (CheckInteractDistance(unit, index)) then
		return 1
	end
	if (XPerl_IsCrossFactionAlly(unit)) then
		return XPerl_InGroupRange(unit) == 1 and 1 or nil
	end
end

-- NormaliseRange(result, unit)
-- IsSpellInRange/IsItemInRange report 1, 0 or nil. A nil means the unit isn't a legal target
-- at all rather than out of range, and a cross faction group member is one of the things that
-- looks like. Only those measure off the roster instead -- anything else we can't cast on
-- (a corpse, say) keeps reporting out of range exactly as it did before.
local function NormaliseRange(result, unit)
	if (result == nil) then
		if (XPerl_IsCrossFactionAlly(unit)) then
			return XPerl_InGroupRange(unit)
		end
		return 0
	...
	if (not range) then
		if (opt.interact) then
			if (opt.interact == 5) then
				range = XPerl_InGroupRange(unit)		-- 40 yards
			else
				range = XPerl_CheckInteractDistance(unit, opt.interact) and 1 or 0
			end
			-- 1 = Inspect = 30 yards
			-- 2 = Trade = 11.11 yards
			-- 3 = Duel = 10 yards
		...
		elseif (opt.spell) then
			range = NormaliseRange(IsSpellInRange(opt.spell, unit), unit)
		elseif (opt.item) then
			range = NormaliseRange(IsItemInRange(opt.item, unit), unit)
		else
			range = 1
		end
	end
...
function XPerl_UpdateSpellRange2(self, overrideUnit, isRaidFrame)
	...
	if (unit) then
		...
		if (rf.enabled and (isRaidFrame or not conf.rangeFinder.raidOnly)) then
			...
			elseif (UnitCanAssist("player", unit) or XPerl_IsCrossFactionAlly(unit)) then
				if (rf.Main.enabled) then
...
function XPerl_BlizzFrameDisable(self)
	-- All-in-one build: if the X-Perl module that replaces this Blizzard frame
	-- is currently disabled, leave the default frame alone.
	if (XPerl_ModuleBlizzKeep and XPerl_ModuleBlizzKeep(self)) then
		return
	end
	if (self) then
